...
namespace Graphics::Vulkan {
	...
	class VulkanSwapchain {
		public:
		VulkanSwapchain(std::shared_ptr<VulkanDevice> device, VkSurfaceKHR surface) : m_device(std::move(device)), m_surface(surface) {};
		...
		inline VkImage GetSwapchainImage(uint8_t index) { return m_swapchainImages[index]; }
