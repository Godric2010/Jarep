...
FinalBlitStep::FinalBlitStep(VkDevice device, VkPhysicalDevice physicalDevice,
                             const std::reference_wrapper<const Pipeline::VulkanSwapchain> swapchain,
                             std::function<uint32_t()> getFramebufferIndex): m_swapchainRef(swapchain) {
	m_device = device;
...
void FinalBlitStep::createRenderPass() {
	Pipeline::RenderPassConfig config = {
		...
		.outputLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR
	};
...
void FinalBlitStep::createFramebuffers() {
	...
	for (auto&imageView: m_swapchainRef.get().getImageViews()) {
